  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {screen === 'feedback' && <FeedbackScreen foodRating={foodRating} setFoodRating={setFoodRating} serviceRating={serviceRating} setServiceRating={setServiceRating} tags={tags} toggleTag={toggleTag} comment={comment} setComment={setComment} onSend={sendFeedback}/>} 
...
function FeedbackScreen({ foodRating, setFoodRating, serviceRating, setServiceRating, tags, toggleTag, comment, setComment, onSend }: { foodRating:number; setFoodRating:(v:number)=>void; serviceRating:number; setServiceRating:(v:number)=>void; tags:string[]; toggleTag:(v:string)=>void; comment:string; setComment:(v:string)=>void; onSend:()=>void }) {
  return <>
    ...
    <View style={styles.feedbackQuestion}><Text style={styles.feedbackTitle}>A comida estava boa?</Text><Text style={styles.feedbackSub}>Sua avaliação é anônima.</Text></View>
    <View style={styles.moodRow}><Mood active={foodRating>=4} icon="happy-outline" label="Boa" onPress={()=>setFoodRating(5)}/><Mood active={foodRating===3} icon="remove-circle-outline" label="Regular" onPress={()=>setFoodRating(3)}/><Mood danger active={foodRating<=2} icon="sad-outline" label="Ruim" onPress={()=>setFoodRating(1)}/></View>
    ...
    <Pressable style={styles.primary} onPress={onSend}><Text style={styles.primaryText}>Enviar avaliação</Text></Pressable>
  </>
}
